...
CREATE EXTENSION IF NOT EXISTS "pg_trgm";   -- for full-text search on names/titles
...
INSERT INTO public.programs (body, full_name, country, description) VALUES
  ...
  ('CFA',  'Chartered Financial Analyst',                    'United States of America',
   'Investment management designation with 3 progressive exam levels.'),
  ('CMA',  'Certified Management Accountant',                'United States of America',
...
-- Seed program levels
INSERT INTO public.program_levels (program_id, label, sequence_no) VALUES
  -- ACCA
  ((SELECT id FROM public.programs WHERE body='ACCA'), 'Applied Knowledge',      1),
  ((SELECT id FROM public.programs WHERE body='ACCA'), 'Applied Skills',         2),
  ((SELECT id FROM public.programs WHERE body='ACCA'), 'Strategic Professional', 3),
  -- CFA
  ((SELECT id FROM public.programs WHERE body='CFA'),  'Level I',   1),
  ((SELECT id FROM public.programs WHERE body='CFA'),  'Level II',  2),
  ((SELECT id FROM public.programs WHERE body='CFA'),  'Level III', 3),
  -- CMA
  ((SELECT id FROM public.programs WHERE body='CMA'),  'Part 1', 1),
  ((SELECT id FROM public.programs WHERE body='CMA'),  'Part 2', 2);
...
CREATE TABLE public.lectures (
  ...
  video_provider    TEXT    DEFAULT 'youtube' CHECK (video_provider IN ('youtube','cloudfront','bunny','vimeo')),
...
CREATE TABLE public.course_pricing (
  ...
  discount_pct      NUMERIC(5,2) GENERATED ALWAYS AS (
    ...
    THEN ROUND(((base_price - discounted_price) / base_price) * 100, 2)
    ELSE 0 END
  ) STORED,
...
CREATE TABLE public.watch_hours_daily (
  ...
  UNIQUE (user_id, course_id, watch_date)
);
...
CREATE TABLE public.quizzes (
  ...
  show_answers_after TEXT    DEFAULT 'submit' CHECK (show_answers_after IN ('submit','never','pass')),
  is_published     BOOLEAN   NOT NULL DEFAULT FALSE,
  created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
COMMENT ON TABLE public.live_streams IS 'Live class sessions linked to a course.';
...
COMMENT ON TABLE public.chat_messages IS 'Live stream chat messages.';
...
SELECT
  ...
  CASE WHEN au.email = 'amarbiradar147@gmail.com' THEN 'admin'::user_role
...
CREATE POLICY "admin_all_program_levels"
  ON public.program_levels FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY "public_read_program_levels"
  ON public.program_levels FOR SELECT TO anon, authenticated
  USING (TRUE);
...
CREATE POLICY "instructor_own_courses"
  ON public.courses FOR ALL TO authenticated
  USING (instructor_id = auth.uid() AND public.is_instructor())
...
CREATE POLICY "enrolled_read_chapters"
...
CREATE POLICY "admin_all_lectures"
  ON public.lectures FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY "public_read_preview_lectures"
...
CREATE POLICY "admin_all_lecture_resources"
  ON public.lecture_resources FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY "enrolled_read_lecture_resources"
...
CREATE POLICY "public_read_free_materials"
  ON public.study_materials FOR SELECT TO anon, authenticated
  USING (access_level = 'free' AND is_published = TRUE);

CREATE POLICY "enrolled_read_enrolled_materials"
  ON public.study_materials FOR SELECT TO authenticated
  USING (
    is_published = TRUE AND
    access_level IN ('enrolled','premium') AND
    EXISTS (
      SELECT 1 FROM public.enrollments e
      WHERE e.user_id = auth.uid()
...
CREATE POLICY "admin_all_payment_plans"
  ON public.payment_plans FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY "public_read_active_plans"
  ON public.payment_plans FOR SELECT TO anon, authenticated
  USING (is_active = TRUE);
...
CREATE POLICY "admin_all_instalments"
  ON public.instalment_schedule FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY "user_read_own_instalments"
  ON public.instalment_schedule FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.enrollments e
      WHERE e.id = instalment_schedule.enrollment_id
        AND e.user_id = auth.uid()
    )
  );
...
CREATE POLICY "admin_all_watch_hours"
  ON public.watch_hours_daily FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY "user_own_watch_hours"
  ON public.watch_hours_daily FOR ALL TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "enrolled_read_quizzes"
...
CREATE POLICY "enrolled_read_options"
...
CREATE POLICY "user_own_quiz_answers"
  ON public.quiz_answers FOR ALL TO authenticated
  USING (
    EXISTS (
...
CREATE POLICY "user_read_own_cert"
...
CREATE POLICY "admin_all_streams"
  ON public.live_streams FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY "enrolled_read_streams"
...
CREATE POLICY "enrolled_read_chat"
  ON public.chat_messages FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.live_streams ls
      JOIN public.enrollments e ON e.course_id = ls.course_id
      WHERE ls.id = chat_messages.live_stream_id
        AND e.user_id = auth.uid()
        AND e.status = 'active'
    )
  );

CREATE POLICY "enrolled_insert_chat"
...
CREATE POLICY "enrolled_insert_review"
...
CREATE POLICY "admin_only_yt_select"
  ON public.youtube_tokens FOR SELECT TO authenticated
  USING ((auth.jwt() ->> 'email') = 'amarbiradar147@gmail.com');

CREATE POLICY "admin_only_yt_insert"
  ON public.youtube_tokens FOR INSERT TO authenticated
  WITH CHECK ((auth.jwt() ->> 'email') = 'amarbiradar147@gmail.com');

CREATE POLICY "admin_only_yt_update"
  ON public.youtube_tokens FOR UPDATE TO authenticated
  USING ((auth.jwt() ->> 'email') = 'amarbiradar147@gmail.com');

CREATE POLICY "admin_only_yt_delete"
  ON public.youtube_tokens FOR DELETE TO authenticated
  USING ((auth.jwt() ->> 'email') = 'amarbiradar147@gmail.com');
...
COMMENT ON TABLE public.topics IS
  'Topics within a subject/paper. e.g., ACCA BT → "The Business Organisation", "Leadership".';
...
COMMENT ON TABLE public.sub_topics IS
  'Optional granular sub-topics within a topic.';
...
COMMENT ON TABLE public.bank_questions IS
  'Central question bank — reusable questions independent of any quiz.';
...
COMMENT ON TABLE public.bank_question_options IS
  'Options for bank questions (MCQ, Multiple Select, True/False, A&R).';
...
COMMENT ON TABLE public.quiz_bank_links IS
  'Tracks which quiz questions originated from the bank. Enables sync-from-bank.';
...
COMMENT ON TABLE public.bank_import_jobs IS
  'Import pipeline jobs for bulk importing questions into the question bank.';
...
COMMENT ON TABLE public.bank_import_question_map IS
  'Maps each imported question back to its source import job and position.';
...
CREATE TABLE public.bank_question_stats (
  ...
  correct_rate    NUMERIC(5,2) GENERATED ALWAYS AS (
    ...
    THEN ROUND((correct_count::NUMERIC / total_attempts) * 100, 2)
    ELSE NULL END
  ) STORED,
...
COMMENT ON TABLE public.bank_question_stats IS
  'Running performance stats per bank question (updated after each quiz attempt).';
...
CREATE INDEX idx_bank_q_text_search     ON public.bank_questions
...
CREATE POLICY "admin_all_subtopics"          ON public.sub_topics               FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "admin_all_bank_questions"     ON public.bank_questions           FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "admin_all_bank_options"       ON public.bank_question_options    FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "admin_all_bank_imports"       ON public.bank_import_jobs         FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "admin_all_bank_import_map"    ON public.bank_import_question_map FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "admin_all_bank_stats"         ON public.bank_question_stats      FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "admin_all_bank_links"         ON public.quiz_bank_links          FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

-- Enrolled students can read verified active bank questions for their program
CREATE POLICY "student_read_bank_questions"
...
CREATE POLICY "public_read_topics"     ON public.topics     FOR SELECT TO anon, authenticated USING (is_active = TRUE);
CREATE POLICY "public_read_subtopics"  ON public.sub_topics FOR SELECT TO anon, authenticated USING (is_active = TRUE);

-- Realtime for admin
ALTER PUBLICATION supabase_realtime ADD TABLE public.bank_questions;
ALTER PUBLICATION supabase_realtime ADD TABLE public.bank_import_jobs;
...
INSERT INTO public.programs (body, full_name, country, description, is_active)
VALUES
  (
    'ACCA',
...
INSERT INTO public.program_levels (program_id, label, sequence_no)
VALUES
  -- ACCA
  ((SELECT id FROM public.programs WHERE body = 'ACCA'), 'Applied Knowledge',      1),
  ((SELECT id FROM public.programs WHERE body = 'ACCA'), 'Applied Skills',         2),
...
ON CONFLICT (program_id, label) DO UPDATE
  SET
...
INSERT INTO public.subjects (program_level_id, code, name, description, sequence_no, is_active)
VALUES
  (
    (SELECT pl.id FROM public.program_levels pl
     JOIN public.programs p ON p.id = pl.program_id
     WHERE p.body = 'ACCA' AND pl.label = 'Applied Knowledge'),
    'BT',
...
-- ────────────────────────────────────────────────────────────
  ...
  (
    (SELECT pl.id FROM public.program_levels pl
     JOIN public.programs p ON p.id = pl.program_id
     WHERE p.body = 'ACCA' AND pl.label = 'Applied Skills'),
    'LW',
...
-- ────────────────────────────────────────────────────────────
  ...
  (
    (SELECT pl.id FROM public.program_levels pl
     JOIN public.programs p ON p.id = pl.program_id
     WHERE p.body = 'ACCA' AND pl.label = 'Strategic Professional'),
    'SBL',
...
-- ────────────────────────────────────────────────────────────
  ...
  (
    (SELECT pl.id FROM public.program_levels pl
     JOIN public.programs p ON p.id = pl.program_id
     WHERE p.body = 'CFA' AND pl.label = 'Level I'),
    'EPS-L1',
...
-- ────────────────────────────────────────────────────────────
  ...
  (
    (SELECT pl.id FROM public.program_levels pl
     JOIN public.programs p ON p.id = pl.program_id
     WHERE p.body = 'CFA' AND pl.label = 'Level II'),
    'EPS-L2',
    'Ethical and Professional Standards',
...
-- ────────────────────────────────────────────────────────────
  ...
  (
    (SELECT pl.id FROM public.program_levels pl
     JOIN public.programs p ON p.id = pl.program_id
     WHERE p.body = 'CFA' AND pl.label = 'Level III'),
    'EPS-L3',
    'Ethical and Professional Standards',
...
-- ────────────────────────────────────────────────────────────
  ...
  (
    (SELECT pl.id FROM public.program_levels pl
     JOIN public.programs p ON p.id = pl.program_id
     WHERE p.body = 'CMA' AND pl.label = 'Part 1'),
    'ECA',
...
-- ────────────────────────────────────────────────────────────
  ...
  (
    (SELECT pl.id FROM public.program_levels pl
     JOIN public.programs p ON p.id = pl.program_id
     WHERE p.body = 'CMA' AND pl.label = 'Part 2'),
    'FA',
    'Financial Statement Analysis',
...
ON CONFLICT (program_level_id, code) DO UPDATE
  SET
    name        = EXCLUDED.name,
    description = EXCLUDED.description,
    sequence_no = EXCLUDED.sequence_no,
...
SET avg_rating = COALESCE((
  ...
  WHERE r.course_id = c.id AND r.is_approved = TRUE
), 0);
...
SET usage_count = (
  ...
  WHERE l.bank_question_id = bq.id
);
...
SELECT
  ...
  ROUND(AVG(qa.score) FILTER (WHERE qa.status='submitted'), 2) AS avg_score,
  ROUND(
    100.0 * COUNT(qa.id) FILTER (WHERE qa.passed = TRUE AND qa.status='submitted')
...
COMMENT ON VIEW public.v_quiz_summary IS
  'Live quiz stats — question_count, total_marks, attempt stats. Never stale.';
...
SELECT
  ...
  COALESCE(ROUND(AVG(r.rating) FILTER (WHERE r.is_approved), 2), 0) AS avg_rating,
...
COMMENT ON VIEW public.v_course_summary IS
  'Live course stats — chapter, lecture, enrollment, rating counts. Never stale.';
...
CREATE INDEX IF NOT EXISTS idx_instalments_due_status
  ON public.instalment_schedule(due_date, status)
  WHERE status = 'pending';

CREATE INDEX IF NOT EXISTS idx_quiz_attempts_status
  ON public.quiz_attempts(status, quiz_id);

CREATE INDEX IF NOT EXISTS idx_quiz_attempts_passed
  ON public.quiz_attempts(passed, quiz_id)
  WHERE status = 'submitted';

CREATE INDEX IF NOT EXISTS idx_payments_instalment
  ON public.payments(enrollment_id, instalment_number)
  WHERE instalment_number IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_bank_q_usage
  ON public.bank_questions(usage_count DESC)
...
FROM (
  SELECT id, total_marks, passing_marks FROM public.quizzes
) sub
WHERE sub.id = qa.quiz_id
  AND qa.total_marks = 0;  -- only fix rows that were never snapshotted

-- Recompute passed flag
UPDATE public.quiz_attempts
SET passed = (score >= passing_marks_snapshot)
WHERE status IN ('submitted', 'timed_out');

-- Recompute bank usage counts
UPDATE public.bank_questions bq
SET usage_count = (
  SELECT COUNT(*) FROM public.quiz_bank_links l WHERE l.bank_question_id = bq.id
);
