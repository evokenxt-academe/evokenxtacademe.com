...
export function CourseDetailContent({ slug }: CourseDetailContentProps) {
  ...
  const { data: dbCourse, isLoading, error, refetch } = useCourseBySlug(slug);
  ...
  if (error || !course) {
    return (
      <CourseDetailError error={error} slug={slug} onRetry={handleRetry} />
    );
  }
