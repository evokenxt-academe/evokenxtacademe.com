        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 bg-muted/20 rounded-lg border border-dashed">
            ...
            <Button variant="link" asChild className="mt-2">
              <Link href={`/admin/quizzes/${quizId}/builder`}>Go to Builder</Link>
            </Button>
          </div>
        ) : (
          questions.map((q, idx) => (
            <Card key={q.id}>
              ...
              <CardContent className="space-y-4">
                <div className="prose prose-sm dark:prose-invert max-w-none font-medium text-lg mb-4" dangerouslySetInnerHTML={{ __html: q.question_text }} />
                
                {/* Assertion Reasoning Specific */}
                {q.type === "assertion_reasoning" && (
                  <div className="space-y-3 mb-4 p-4 rounded-lg bg-muted/30 border">
                    <div className="space-y-1">
                      <div className="text-xs font-bold uppercase text-muted-foreground">Assertion (A)</div>
                      <div className="text-sm font-medium">{q.assertion_text}</div>
                    </div>
                    <div className="space-y-1">
                      <div className="text-xs font-bold uppercase text-muted-foreground">Reason (R)</div>
                ...
                {["mcq", "multiple_select", "true_false", "assertion_reasoning"].includes(q.type) && (
                  <div className="grid gap-2">
                    {q.options?.map((opt: any) => (
                      ...
                      >
                        <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-[10px] font-bold ${opt.is_correct ? "bg-green-500 border-green-500 text-white" : "text-muted-foreground"}`}>
                          {String.fromCharCode(65 + (opt.position || 0))}
                        </div>
                        <div className="flex-1 text-sm font-medium">{opt.option_text}</div>
                        {opt.is_correct && <CheckCircle2 className="h-4 w-4 text-green-500" />}
                      </div>
                    ))}
                  </div>
                )}
                ...
                {q.type === "numerical" && (
                  <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-900">
                    <div className="text-xs font-bold text-green-700 dark:text-green-300 uppercase mb-1">Correct Answer</div>
                    <div className="text-lg font-mono font-bold text-green-600 dark:text-green-400">
                      {q.numerical_answer} 
                      {q.numerical_tolerance ? <span className="text-xs font-normal ml-2">(±{q.numerical_tolerance})</span> : ""}
                    </div>
                  </div>
                )}
                ...
                {q.model_answer && (
                  <div className="mt-4 p-4 rounded-lg bg-muted/50 border border-border">
                    <div className="text-xs font-bold uppercase text-muted-foreground mb-2">Model Answer</div>
                    <div className="text-sm prose prose-sm dark:prose-invert" dangerouslySetInnerHTML={{ __html: q.model_answer }} />
                  </div>
                )}
                ...
                {q.explanation && (
                  <div className="mt-4 p-4 rounded-lg bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30">
                    ...
                    <div className="text-sm text-blue-600 dark:text-blue-400 prose prose-sm dark:prose-invert" dangerouslySetInnerHTML={{ __html: q.explanation }} />
                  </div>
                )}
              </CardContent>
            </Card>
