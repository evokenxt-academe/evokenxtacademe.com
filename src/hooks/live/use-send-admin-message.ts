...
async function sendAdminChatMessage(payload: SendMessageInput): Promise<LiveChatMessage> {
    const response = await fetch(`/api/admin/live-streams/${payload.streamId}`, {
        method: "POST",
...
export function useSendAdminMessage() {
    ...
    const mutation = useMutation({
        mutationFn: async (payload: SendMessageInput) => {
            ...
            return sendAdminChatMessage(payload)
        },
    })
