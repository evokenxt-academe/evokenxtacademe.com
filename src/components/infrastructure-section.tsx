...
const regions = [
  { name: "Asia & Middle East", nodes: 45, status: "active" },
  { name: "Europe & UK", nodes: 32, status: "active" },
  { name: "Africa", nodes: 28, status: "active" },
  { name: "Americas & Caribbean", nodes: 15, status: "active" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                ACCA is accepted in over 180 countries worldwide.
                Study with us from anywhere and earn a globally respected qualification.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-8xl lg:text-[10rem] font-display leading-none">180+</span>
                <span className="text-2xl text-muted-foreground">countries</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                ACCA members and students spread across continents, building careers in finance and accounting.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">95%</span>
              <span className="block text-sm text-muted-foreground mt-2">Exam Pass Rate</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">13</span>
              <span className="block text-sm text-muted-foreground mt-2">ACCA Papers Covered</span>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4 transition-all duration-1000 delay-300 ${
          ...
          {regions.map((region, index) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground">{region.nodes} study centres</span>
            </div>
          ))}
        </div>
      </div>
